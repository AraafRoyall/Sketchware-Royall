...
*/	
	...
	private void initializeEventsCodeGenerator() {
		initializeMethodCode.add(getActivityAssign(isFragment, isDialogFragment, isBottomDialogFragment));
		eventManager = new Hx(buildConfig, projectFileBean, projectDataManager);
		addImports(eventManager.getImports());
	}
	private String getActivityAssign(boolean isFragment, boolean isDialog, boolean isBottomDialog) {
		if (isFragment || isDialog || isBottomDialog) {
...
*/	
	...
	private void addFieldsDeclaration() {
		...
		fields.add("private Activity activity;");
	}
